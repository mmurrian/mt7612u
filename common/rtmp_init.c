...
static void load_eeprom(struct rtmp_adapter *pAd)
{
...
VOID NICReadEEPROMParameters(struct rtmp_adapter *pAd)
{
	...
	Addr01 = mt76u_read_eeprom(pAd, MT_EE_MAC_ADDR);
	Addr23 = mt76u_read_eeprom(pAd, MT_EE_MAC_ADDR+2);
	Addr45 = mt76u_read_eeprom(pAd, MT_EE_MAC_ADDR+4);
	...
	Version.word = mt76u_read_eeprom(pAd, MT_EE_VERSION);
	...
	value = mt76u_read_eeprom(pAd, MT_EE_XTAL_TRIM_1);
	...
	/* */
	if (IS_MT76x2U(pAd)) {
		value = mt76u_read_eeprom(pAd, MT_EE_RSSI_OFFSET_2G_0);
...
/*			if (IS_RT2860(pAd))  RT2860 supports 3 Rx and the 2.4 GHz RSSI #2 offset is in the EEPROM 0x48*/
	...
	value = mt76u_read_eeprom(pAd, MT_EE_LNA_GAIN);
	...
	if (IS_MT76x2U(pAd)) {
		value = mt76u_read_eeprom(pAd, MT_EE_RSSI_OFFSET_5G_0);
	...
	value = mt76u_read_eeprom(pAd, MT_EE_RSSI_OFFSET_5G_1);
...
VOID NICUpdateFifoStaCounters(struct rtmp_adapter *pAd)
{
	...
	do {
#ifdef FIFO_EXT_SUPPORT
		StaFifoExt.word = mt76u_reg_read(pAd, TX_STA_FIFO_EXT);
#endif /* FIFO_EXT_SUPPORT */
		StaFifo.word = mt76u_reg_read(pAd, TX_STA_FIFO);
