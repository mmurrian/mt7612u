...
typedef struct __CMD_RTPRIV_IOCTL_80211_BEACON {
        ...
        u8 *beacon_head;
		u8 *beacon_tail;
        uint32_t beacon_head_len; /* Before TIM IE */
		uint32_t beacon_tail_len; /* After TIM IE */
...
typedef struct __CMD_RTPRIV_IOCTL_80211_SURVEY {
	...
	VOID *pCfg80211;
/*	uint64_t ChannelTime; */ /* idle + busy, not support */
...
typedef struct __RT_CMD_USB_MORE_FLAG_CONFIG {
	IN uint32_t VendorID;
	IN uint32_t ProductID;
...
typedef struct __RT_CMD_PCIE_INIT {
	...
	IN VOID *pPciDev;
	IN uint32_t ConfigDeviceID;
	IN uint32_t ConfigSubsystemVendorID;
	IN uint32_t ConfigSubsystemID;
} RT_CMD_PCIE_INIT;
...
typedef struct __RT_CMD_AP_IOCTL_CONFIG {
	...
	IN ULONG priv_flags;
	IN char *pCmdData;
	IN int32_t CmdId_RTPRIV_IOCTL_SET;
	IN char *name;
	IN INT apidx;
...
typedef struct __RT_CMD_STA_IOCTL_SECURITY {
	INOUT CHAR *pData;
	INOUT uint16_t length;
	IN int32_t KeyIdx;
	IN int32_t MaxKeyLen;
